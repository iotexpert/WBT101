...
static const uint8_t serviceUUID[] = {0xF0 ,0x34 ,0x9B ,0x5F ,0x80 ,0x00 ,0x00 ,0x80 ,0x00 ,0x10 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 };
...
static const uint8_t buttonUUID[] = {0xF2u,0x34u,0x9Bu,0x5Fu,0x80u,0x00u,0x00u,0x80u,0x00u,0x10u,0x00u,0x00u,0x00u,0x00u,0x00u,0x00u };
...
wiced_bt_gatt_status_t readLed()
{
    ...
    param.by_handle.handle = ledChar.valHandle;
...
wiced_bt_gatt_status_t gatt_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data)
{
    ...
    static uint16_t charStartHandle=0;
    static uint16_t charEndHandle=0;
    ...
    case GATT_DISCOVERY_RESULT_EVT:
        ...
        if(p_data->discovery_result.discovery_type == GATT_DISCOVER_CHARACTERISTICS)
        {
            ...
            if(memcmp(ledUUID,GATT_DISCOVERY_RESULT_CHARACTERISTIC_UUID128(p_data),16) == 0)
            {
                ledChar.startHandle = p_data->discovery_result.discovery_data.characteristic_declaration.handle;
                ledChar.valHandle = GATT_DISCOVERY_RESULT_CHARACTERISTIC_VALUE_HANDLE(p_data);
            }

            if(memcmp(buttonUUID,GATT_DISCOVERY_RESULT_CHARACTERISTIC_UUID128(p_data),16) == 0)
            {
                buttonChar.startHandle = p_data->discovery_result.discovery_data.characteristic_declaration.handle;
