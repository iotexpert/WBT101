...
extern const wiced_bt_cfg_settings_t hello_sensor_cfg_settings;
extern const wiced_bt_cfg_buf_pool_t hello_sensor_cfg_buf_pools[];
...
APPLICATION_START( )
{
    ...
    //For the 20 MHz board set the UART type as WICED_ROUTE_DEBUG_TO_DBG_UART
...
void hello_sensor_set_advertisement_data(void)
{
    ...
    uint8_t  hci_control_le_local_name[]      = {'h', 'e', 'l', 'l', 'o', 0x00, 0x00};
...
void hello_sensor_timeout( uint32_t count )
{
    ...
    //GJL WICED_BT_TRACE("hello_sensor_timeout: %d\n", hello_sensor_state.timer_count);
}
...
void hello_sensor_fine_timeout( uint32_t finecount )
{
    ...
    {
        //GJL WICED_BT_TRACE("hello_sensor_fine_timeout: %d\n", hello_sensor_state.fine_timer_count);
    }
}
...
{
    WICED_BT_TRACE( "encryp change bd ( %B ) res: %d\n ", hello_sensor_hostinfo.bdaddr,  result);
...
wiced_result_t hello_sensor_management_cback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    WICED_BT_TRACE("BT_Callbackk: %d:\t", event );
...
{
    WICED_BT_TRACE("\tGJL: GATT Callback: %d\n", event);
