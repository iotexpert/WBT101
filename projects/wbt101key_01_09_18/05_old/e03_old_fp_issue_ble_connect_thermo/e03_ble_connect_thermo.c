...
/* Thermometer measurement characteristic constants */
/* Bit 0 - Celsius=0, Bit 1 - No Time Stamp=0, Bit 3 - No Type=0 */
#define THERMO_FLAG_VAL (0x0)
...
uint16_t capSenseNotify = 0;    // Start with notifications off
wiced_bool_t capSenseChangeFlag = WICED_FALSE; // Flag is set when a CapSense value changes

/* Temperature measurement characteristic */
/* The temperature uses indications rather than notifications which means that the server */
/* guarantees that the message gets to the client */
struct {
    uint8_t Flag;
    //GJL FP lib float Val;
    int32_t Val;
} __attribute__((packed)) thermoChar;
//GJL FP lib float thermoValPrev = 0.0;
int32_t thermoValPrev = 0;
uint16_t thermoIndicate = 0;    // Start with indications off
uint8_t thermoChangeFlag = 0;   // Flag is incremented when a temperature value changes
uint8_t sensorLoc = BODY;
...
const uint8_t gatt_database[]=
{
    ...
    // Declare Health Thermometer Service
    PRIMARY_SERVICE_UUID16( HANDLE_THERMOMETER_SERVICE, UUID_SERVICE_HEALTH_THERMOMETER),

        // Declare characteristic used to indicate a change
        CHARACTERISTIC_UUID16( HANDLE_THERMOMETER_SERVICE_CHAR_MEASUREMENT, HANDLE_THERMOMETER_SERVICE_CHAR_MEASUREMENT_VAL,
                UUID_CHARACTERISTIC_TEMPERATURE_MEASUREMENT, LEGATTDB_CHAR_PROP_READ | LEGATTDB_CHAR_PROP_INDICATE, LEGATTDB_PERM_READABLE ),
            ...
            CHAR_DESCRIPTOR_UUID16_WRITABLE( HANDLE_THERMOMETER_SERVICE_CHAR_CFG_DESC, UUID_DESCRIPTOR_CLIENT_CHARACTERISTIC_CONFIGURATION,
            LEGATTDB_PERM_READABLE | LEGATTDB_PERM_WRITE_REQ | LEGATTDB_PERM_AUTH_READABLE | LEGATTDB_PERM_AUTH_WRITABLE),

        // Declare characteristic for temperature type (sensor location)
        CHARACTERISTIC_UUID16( HANDLE_THERMOMETER_SERVICE_CHAR_TYPE, HANDLE_THERMOMETER_SERVICE_CHAR_TYPE_VAL,
               UUID_CHARACTERISTIC_TEMPERATURE_TYPE, LEGATTDB_CHAR_PROP_READ, LEGATTDB_PERM_READABLE ),
...
attribute_t gauAttributes[] =
{
    ...
    { HANDLE_THERMOMETER_SERVICE_CHAR_MEASUREMENT_VAL, sizeof(thermoChar), &thermoChar},
    { HANDLE_THERMOMETER_SERVICE_CHAR_CFG_DESC, sizeof(thermoIndicate), &thermoIndicate},
    { HANDLE_THERMOMETER_SERVICE_CHAR_TYPE_VAL, sizeof(sensorLoc), &sensorLoc},
    { HANDLE_CAPSENSE_SERVICE_CHAR_BUTTONS_VAL, sizeof(capSenseVal), capSenseVal},
    { HANDLE_CAPSENSE_SERVICE_CHAR_CFG_DESC, sizeof(capSenseNotify), &capSenseNotify},
...
void application_init( void );  /* Initialize the application */
void getPsocI2c(uint32_t arg);  /* This is a timer callback function used to get I2C data from PSoC */
void bt_set_advertisement_data( void ); /* Setup BLE advertisement packets */
...
wiced_bt_gatt_status_t gatts_req_read_handler( uint16_t conn_id, wiced_bt_gatt_read_t * p_read_data )
{
    ...
    WICED_BT_TRACE("\t\tGATT Read Handler: conn_id:%d hdl:%x offset:%d len:%d\r\n", conn_id, p_read_data->handle, p_read_data->offset, attr_len_to_copy );
...
wiced_bt_gatt_status_t gatts_req_write_handler( uint16_t conn_id, wiced_bt_gatt_write_t * p_data )
{
    ...
    WICED_BT_TRACE("\t\tGATT Write Handler: conn_id:%d hdl:0x%x prep:%d offset:%d len:%d val[0]:%x\r\n", conn_id, p_data->handle, p_data->is_prep, p_data->offset, p_data->val_len, p_attr[0]);

    switch ( p_data->handle )
    {
    /* By writing into Characteristic Client Configuration descriptor
     * peer can enable or disable indications */
    case HANDLE_THERMOMETER_SERVICE_CHAR_CFG_DESC:
        if ( p_data->val_len != 2 )
        {
            return WICED_BT_GATT_INVALID_ATTR_LEN;
        }
        thermoIndicate = p_attr[0] | ( p_attr[1] << 8 );
        /* If indications have just been enalbed, need to send initial temperature */
        if(thermoIndicate & GATT_CLIENT_CONFIG_INDICATION)
        {
            thermoChangeFlag++;
        }
        break;
    ...
    case HANDLE_CAPSENSE_SERVICE_CHAR_CFG_DESC:
        ...
        capSenseNotify = p_attr[0] | ( p_attr[1] << 8 );
        break;
...
wiced_bt_gatt_status_t gatts_req_conf_handler( uint16_t conn_id, uint16_t handle )
{
    WICED_BT_TRACE( "\t\tGATT Indication Confirmation Handler: conn %d hdl %d\r\n", conn_id, handle );

    /* We might need to send more indications */
    if ( thermoChangeFlag > 0)
    {
        send_message();
    }
...
void send_message( void )
{
    ...
    //WICED_BT_TRACE( "GJL SEND MESSAGE: indicate: %d, chg flag: %d, tempval: %d\r\n", thermoIndicate, thermoChangeFlag, thermoChar.Val );

    /* Send data only if notifications/indications have been enabled and if value has changed */
    if ((thermoIndicate & GATT_CLIENT_CONFIG_INDICATION ) && (thermoChangeFlag > 0))
    {
        thermoChangeFlag--;
        wiced_bt_gatt_send_indication(connection_id, HANDLE_THERMOMETER_SERVICE_CHAR_MEASUREMENT_VAL, sizeof(thermoChar), (uint8_t*)&thermoChar );
        WICED_BT_TRACE( "\tSend Indication: sending Thermometer value: %x, Change Flag: %d\r\n", thermoChar.Val, thermoChangeFlag );
    }

    if((capSenseNotify & GATT_CLIENT_CONFIG_NOTIFICATION ) && capSenseChangeFlag)
    {
        capSenseChangeFlag = WICED_FALSE;
...
/*
 * This function initializes the application. It is executed in the BTM_ENABLED_EVT management callback.
 */
void application_init( void )
{
    ...
    /* Initialize temperature structure values */
    thermoChar.Flag = THERMO_FLAG_VAL;
    thermoChar.Val = 0;
...
/* The function invoked on timeout of the timer. It gets temperature and CapSense button data from the PSoC*/
void getPsocI2c( uint32_t arg )
{
    ...
    /* Copy Temperature value from I2C register to the temperature GATT locations */
    thermoChar.Val = i2cReadBuf.temp;

    /* If temperature has changed and indications are enabled, then increment the
     * change flag so that an indication of the new temperature will be sent. */
    /* A counter is used so that if the temperature changes before the prior indication
    * is sent we won't miss sending the new value. */
    /* This is only incremented if indications are set so that we don't store up send requests
     * prior to turning on indications. */
    if ((thermoValPrev != thermoChar.Val) && (thermoIndicate & GATT_CLIENT_CONFIG_INDICATION))
    {
        thermoChangeFlag++;
        thermoValPrev = thermoChar.Val;
    }

    /* Send data to the peer (only gets sent if peer has registered for notifications/indications) */
    send_message();
}
