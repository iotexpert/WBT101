 ...
 * Function Prototypes
 ******************************************************************/
static void                   ex06_ble_multi_app_init               ( void );
static wiced_bt_dev_status_t  ex06_ble_multi_management_callback    ( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data );
static void                   ex06_ble_multi_set_advertisement_data ( void );
static void                   ex06_ble_multi_advertisement_stopped  ( void );
static void                   ex06_ble_multi_reset_device           ( void );
/* GATT Registration Callbacks */
static wiced_bt_gatt_status_t ex06_ble_multi_write_handler          ( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id );
static wiced_bt_gatt_status_t ex06_ble_multi_read_handler           ( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id );
static wiced_bt_gatt_status_t ex06_ble_multi_connect_callback       ( wiced_bt_gatt_connection_status_t *p_conn_status );
static wiced_bt_gatt_status_t ex06_ble_multi_server_callback        ( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data );
static wiced_bt_gatt_status_t ex06_ble_multi_event_handler          ( wiced_bt_gatt_evt_t  event, wiced_bt_gatt_event_data_t *p_event_data );
static uint32_t               hci_control_process_rx_cmd          ( uint8_t* p_data, uint32_t len );
#ifdef HCI_TRACE_OVER_TRANSPORT
static void                   ex06_ble_multi_trace_callback         ( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data );
#endif
 ...
 ******************************************************************/
uint8_t ex06_ble_multi_generic_access_device_name[] = {'k','e','y','_','m','u','l','t'};
uint8_t ex06_ble_multi_generic_access_appearance[]  = {0x00,0x00};
uint8_t ex06_ble_multi_capsense_buttons[]           = {0x04,0x00,0x00};
uint8_t ex06_ble_multi_capsense_buttons_cccd[]      = {0x00,0x00};
...
gatt_db_lookup_table ex06_ble_multi_gatt_db_ext_attr_tbl[] =
{
    /* { attribute handle,                       maxlen, curlen, attribute data } */
    {HDLC_GENERIC_ACCESS_DEVICE_NAME_VALUE,      12,     12,     ex06_ble_multi_generic_access_device_name},
    {HDLC_GENERIC_ACCESS_APPEARANCE_VALUE,       2,      2,      ex06_ble_multi_generic_access_appearance},
    {HDLC_CAPSENSE_BUTTONS_VALUE,                3,      3,      ex06_ble_multi_capsense_buttons},
    {HDLD_CAPSENSE_BUTTONS_CLIENT_CONFIGURATION, 2,      2,      ex06_ble_multi_capsense_buttons_cccd},
};

// Number of Lookup Table Entries
const uint16_t ex06_ble_multi_gatt_db_ext_attr_tbl_size = ( sizeof ( ex06_ble_multi_gatt_db_ext_attr_tbl ) / sizeof ( gatt_db_lookup_table ) );
...
void application_start(void)
{
    ...
    wiced_bt_stack_init(ex06_ble_multi_management_callback, &wiced_bt_cfg_settings, wiced_bt_cfg_buf_pools);
}

/*
 * This function is executed in the BTM_ENABLED_EVT management callback.
 */
void ex06_ble_multi_app_init(void)
{
    uint8_t count;
    ...
    ex06_ble_multi_set_advertisement_data();
    ...
    wiced_bt_gatt_register( ex06_ble_multi_event_handler );
...
/* Set Advertisement Data */
void ex06_ble_multi_set_advertisement_data( void )
{
...
void ex06_ble_multi_advertisement_stopped( void )
{
...
void ex06_ble_multi_reset_device( void )
{
...
wiced_bt_dev_status_t ex06_ble_multi_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:
        ...
        wiced_bt_dev_register_hci_trace(ex06_ble_multi_trace_callback);
#endif
    ...
    case BTM_ENCRYPTION_STATUS_EVT:
        ...
        for(count = 0; count < bondInfo[NUM_BONDED]; count++)
        {
            ...
            if( memcmp(hostinfo.bdaddr,hostinfoTemp.bdaddr,sizeof(hostinfo.bdaddr) ) == 0) /* Matching address */
            {
                ...
                ex06_ble_multi_capsense_buttons_cccd[0] = hostinfo.cccd; /* Set CCCD value from the value that was previously saved in the NVRAM */
    ...
    case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
        ...
        {
            ex06_ble_multi_advertisement_stopped();
        }
        break;
...
/* Get a Value */
wiced_bt_gatt_status_t ex06_ble_multi_get_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t max_len, uint16_t *p_len )
{
    int i = 0;
    ...
    for (i = 0; i < ex06_ble_multi_gatt_db_ext_attr_tbl_size; i++)
    {
        if (ex06_ble_multi_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            // Detected a matching handle in external lookup table
            isHandleInTable = WICED_TRUE;
            // Detected a matching handle in the external lookup table
            if (ex06_ble_multi_gatt_db_ext_attr_tbl[i].cur_len <= max_len)
            {
                // Value fits within the supplied buffer; copy over the value
                *p_len = ex06_ble_multi_gatt_db_ext_attr_tbl[i].cur_len;
                memcpy(p_val, ex06_ble_multi_gatt_db_ext_attr_tbl[i].p_data, ex06_ble_multi_gatt_db_ext_attr_tbl[i].cur_len);
...
/* Set a Value */
wiced_bt_gatt_status_t ex06_ble_multi_set_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t len )
{
    int i = 0;
    wiced_bool_t isHandleInTable = WICED_FALSE;
    ...
    for (i = 0; i < ex06_ble_multi_gatt_db_ext_attr_tbl_size; i++)
    {
        if (ex06_ble_multi_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            validLen = (ex06_ble_multi_gatt_db_ext_attr_tbl[i].max_len >= len);
            if (validLen)
            {
                // Value fits within the supplied buffer; copy over the value
                ex06_ble_multi_gatt_db_ext_attr_tbl[i].cur_len = len;
                memcpy(ex06_ble_multi_gatt_db_ext_attr_tbl[i].p_data, p_val, len);
                res = WICED_BT_GATT_SUCCESS;
...
wiced_bt_gatt_status_t ex06_ble_multi_write_handler( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Write Request */
    status = ex06_ble_multi_set_value(p_write_req->handle, conn_id, p_write_req->p_val, p_write_req->val_len);

    return status;
}

/* Handles Read Requests received from Client device */
wiced_bt_gatt_status_t ex06_ble_multi_read_handler( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Read Request */
    status = ex06_ble_multi_get_value(p_read_req->handle, conn_id, p_read_req->p_val, *p_read_req->p_val_len, p_read_req->p_val_len);

    return status;
}

/* GATT Connection Status Callback */
wiced_bt_gatt_status_t ex06_ble_multi_connect_callback( wiced_bt_gatt_connection_status_t *p_conn_status )
{
    ...
    if ( NULL != p_conn_status )
    {
        ...
        else
        {
            ...
            ex06_ble_multi_capsense_buttons_cccd[0] = 0;
...
wiced_bt_gatt_status_t ex06_ble_multi_server_callback( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATTS_REQ_TYPE_READ:
        status = ex06_ble_multi_read_handler( &p_data->read_req, conn_id );
        break;
    case GATTS_REQ_TYPE_WRITE:
        status = ex06_ble_multi_write_handler( &p_data->write_req, conn_id );
        break;
    }
...
/* GATT Event Handler */
wiced_bt_gatt_status_t ex06_ble_multi_event_handler( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_event_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATT_CONNECTION_STATUS_EVT:
        status = ex06_ble_multi_connect_callback( &p_event_data->connection_status );
        break;
    case GATT_ATTRIBUTE_REQUEST_EVT:
        p_attr_req = &p_event_data->attribute_request;
        status = ex06_ble_multi_server_callback( p_attr_req->conn_id, p_attr_req->request_type, &p_attr_req->data );
        break;
    default:
        status = WICED_BT_GATT_SUCCESS;
        break;
...
void ex06_ble_multi_trace_callback( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data )
{
...
void i2c_read( uint32_t arg )
{
    ...
    for(;;)
    {
        ...
        if(prevVal != buttonVal) /* Only print if value has changed since last time */
        {
            ...
            ex06_ble_multi_capsense_buttons[2] = buttonVal;
            /* If the connection is up and if the client wants notifications, send it */
            if ( connection_id != 0)
            {
                 if(ex06_ble_multi_capsense_buttons_cccd[0] & GATT_CLIENT_CONFIG_NOTIFICATION)
                {
                    wiced_bt_gatt_send_notification(connection_id, HDLC_CAPSENSE_BUTTONS_VALUE, sizeof(ex06_ble_multi_capsense_buttons), ex06_ble_multi_capsense_buttons );
