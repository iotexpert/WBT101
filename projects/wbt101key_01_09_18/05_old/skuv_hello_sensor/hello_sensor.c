...
#define cr_pad_config_adr10                            0x003201ac
...
extern  void                     hello_sensor_passkey_reply (uint32_t passkey);
...
void hello_sensor_application_init( void )
{
    ...
    /* Configure buttons avajilable on the platform (pin should be configured before registering interrupt handler ) */
...
//       wiced_hal_gpio_register_pin_for_interrupt( WICED_GPIO_BUTTON, hello_sensor_interrupt_handler, NULL );
 ...
 //  wiced_hal_gpio_register_pin_for_interrupt( WICED_P04, hello_sensor_interrupt_handler, NULL );
...
void hello_sensor_passkey_reply (uint32_t passkey)
{
   wiced_bt_dev_pass_key_req_reply(WICED_BT_SUCCESS, hello_sensor_state.remote_addr ,passkey);
}
...
{
    WICED_BT_TRACE( "BUTTON INTERRUPT TRIGGERED");
    wiced_hal_gpio_configure_pin( WICED_P28, GPIO_OUTPUT_ENABLE, GPIO_PIN_OUTPUT_HIGH );
//    // Blink as configured
...
//        wiced_bt_dev_confirm_req_reply(WICED_BT_SUCCESS, p_event_data->user_passkey_notification.bd_addr );
    ...
    case BTM_PASSKEY_REQUEST_EVT:
        ...
        WICED_BT_TRACE ("Enter PASSKEY :::::  %d\r\n",passkey_skuv);
        /*
        Read the passkey from the hyperterminal over UART if BTM_IO_CAPABILITIES_KEYBOARD_ONLY
        option is chosen for    p_event_data->pairing_io_capabilities_ble_request.local_io_cap

        hello_sensor_passkey_reply (uint32_t xxxx);
...
wiced_bt_gatt_status_t hello_sensor_gatts_connection_up( wiced_bt_gatt_connection_status_t *p_status )
{
 ...
 //   wiced_bt_set_pairable_mode(WICED_TRUE, 0);
    //GJL - needed for the device to initiate pairing with the peer. Otherwise, need to manual pair from
    //      CySmart
