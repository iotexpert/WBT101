...
void    u8g_init_wiced_i2c_device(uint8_t configure_path, uint8_t address)
{
    i2c_pins = configure_path;
    i2c_address = address;
}
...
uint8_t u8g_com_hw_i2c_fn(u8g_t *u8g, uint8_t msg, uint8_t arg_val, void *arg_ptr)
{
    ...
    switch(msg)
    {
        ...
        case U8G_COM_MSG_INIT:
            if (wiced_hal_i2c_init(i2c_pins) != WICED_SUCCESS)
            {
        ...
        case U8G_COM_MSG_WRITE_BYTE:
            ...
            if (!started)
            {
                wiced_hal_i2c_write(&control, sizeof control, i2c_address);
                //GJL wiced_i2c_write(&i2c_display, WICED_I2C_START_FLAG, &control, sizeof control);
                started = 1;
            }
            else
            {
                wiced_hal_i2c_write(&control, sizeof control, i2c_address);
                //GJL wiced_i2c_write(&i2c_display, 0, &control, sizeof control);
            }
#endif

            result = wiced_hal_i2c_write(&arg_val, sizeof arg_val, i2c_address);
            //GJL result = wiced_i2c_write(&i2c_display, 0, &arg_val, 1);
            if (result != WICED_SUCCESS)
            {
        ...
        case U8G_COM_MSG_WRITE_SEQ_P:
            ...
            result = wiced_hal_i2c_write(arg_ptr, arg_val, i2c_address);
            //GJL result = wiced_i2c_write(&i2c_display, WICED_I2C_STOP_FLAG, arg_ptr, arg_val);
            if (result != WICED_SUCCESS)
            {
